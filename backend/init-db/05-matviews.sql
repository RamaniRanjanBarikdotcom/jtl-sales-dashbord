...
SELECT
  ...
  SUM(gross_revenue) FILTER (WHERE normalized_order_status(status) <> 'cancelled') AS total_revenue,
  SUM(net_revenue) FILTER (WHERE normalized_order_status(status) <> 'cancelled') AS total_net_revenue,
  AVG(gross_revenue) FILTER (WHERE normalized_order_status(status) <> 'cancelled') AS avg_order_value,
...
SELECT
  ...
  SUM(oi.line_total_gross) FILTER (WHERE normalized_order_status(o.status) <> 'cancelled') AS total_revenue,
  SUM(oi.quantity) FILTER (WHERE normalized_order_status(o.status) <> 'cancelled') AS total_units,
  AVG(
    (oi.unit_price_net - oi.unit_cost) / NULLIF(oi.unit_price_net, 0) * 100
  ) FILTER (WHERE normalized_order_status(o.status) <> 'cancelled') AS margin_pct,
