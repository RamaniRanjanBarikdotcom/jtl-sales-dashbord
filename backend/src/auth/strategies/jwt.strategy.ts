...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(payload: RequestUser) {
    ...
    let tenantId = payload.tenantId || user.tenant_id;
    let role = user.role;
    ...
    if (user.role !== 'super_admin') {
      if (!tenantId) throw new UnauthorizedException('No tenant assigned');
    ...
    if (user.role === 'super_admin') {
      return { ...currentPayload, permissions: ['*'] };
    }
    const permissions = membership
      ? (await this.membershipPermissionRepo.find({
          where: { membership_id: membership.id },
          select: { permission_key: true },
        })).map((permission) => permission.permission_key)
      : await this.permissionsService.getEffectivePermissionKeys(payload.sub);
    return { ...currentPayload, permissions };
  }
}
