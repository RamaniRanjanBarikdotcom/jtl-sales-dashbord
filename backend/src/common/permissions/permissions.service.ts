...
export class PermissionsService implements OnModuleInit {
  ...
  normalizePermissionKeys(keys: string[]): string[] {
    return this.normalize(keys);
  }
  ...
  async validatePermissionKeys(keys: string[]): Promise<string[]> {
    const unique = this.normalize(keys);
    await this.resolvePermissionIds(unique);
    return unique;
  }
