...
function normalizeStatusFilter(value?: string): string {
  ...
  if (!status || status === 'all' || status === 'all_orders' || status === 'all orders') return '';
  if (['cancelled', 'canceled', 'storniert', 'storno', 'annulliert', 'void', 'voided'].includes(status)) {
    return 'cancelled';
  }
  if (['returned', 'retour', 'retoure', 'retourniert', 'refund', 'refunded'].includes(status)) {
    return 'returned';
  }
  return status;
}
...
function statusPredicate(column: string, paramIndex: number): string {
  return `($${paramIndex} = '' OR ${normalizedStatusExpr(column)} = $${paramIndex})`;
}

function activeStatusPredicate(column: string): string {
  return `${normalizedStatusExpr(column)} <> 'cancelled'`;
}

function cancelledStatusPredicate(column: string): string {
  return `${normalizedStatusExpr(column)} = 'cancelled'`;
}

function returnedStatusPredicate(column: string): string {
  return `${normalizedStatusExpr(column)} = 'returned'`;
}
      ...
      if (statusFilter || invoiceScope !== 'all' || paymentMethodFilter || channelFilter || platformFilter) {
        ...
        const kpiSql = `
          SELECT
            ...
            COALESCE(ROUND(AVG(gross_revenue) FILTER (WHERE ${activeStatusPredicate('status')})::numeric, 2), 0) AS avg_order_value,
            COUNT(*) FILTER (WHERE ${returnedStatusPredicate('status')})::int                AS total_returns,
            COALESCE(
              ROUND((COUNT(*) FILTER (WHERE ${returnedStatusPredicate('status')}))::numeric / NULLIF(COUNT(*) FILTER (WHERE ${activeStatusPredicate('status')}), 0) * 100, 2),
              0
            )                                                                          AS return_rate
          FROM orders
        ...
        const statusSql = `
          SELECT
            ...
            COALESCE(SUM(gross_revenue) FILTER (WHERE ${cancelledStatusPredicate('status')}), 0) AS cancelled_revenue,
      ...
      const [
        ...
        this.db.query(
          `SELECT ${breakdownExpr} AS region_name,
                  ...
                  COUNT(DISTINCT customer_id) FILTER (WHERE ${activeStatusPredicate('status')})::int AS customers,
                  COUNT(*) FILTER (WHERE ${cancelledStatusPredicate('status')})::int AS cancelled_orders,
                  COALESCE(SUM(gross_revenue) FILTER (WHERE ${cancelledStatusPredicate('status')}), 0)::numeric AS cancelled_revenue,
                  COUNT(*) FILTER (WHERE ${returnedStatusPredicate('status')})::int AS returned_orders,
        ...
        this.db.query(
          `SELECT
             ...
             COUNT(*)::int AS orders,
             COUNT(*) FILTER (WHERE ${activeStatusPredicate('status')})::int AS good_orders,
             COUNT(*) FILTER (WHERE ${cancelledStatusPredicate('status')})::int AS bad_orders,
             COALESCE(SUM(gross_revenue) FILTER (WHERE ${activeStatusPredicate('status')}), 0)::numeric AS revenue,
             COALESCE(ROUND(AVG(gross_revenue) FILTER (WHERE ${activeStatusPredicate('status')})::numeric, 2), 0)::numeric AS avg_order_value
           FROM orders
           WHERE tenant_id=$1 AND order_date BETWEEN $2 AND $3
        ...
        this.db.query(
          `SELECT
             ...
             COALESCE(SUM(oi.quantity) FILTER (WHERE ${activeStatusPredicate('o.status')}), 0)::numeric AS quantity,
             COUNT(DISTINCT o.jtl_order_id) FILTER (WHERE ${activeStatusPredicate('o.status')})::int AS orders,
             COALESCE(
               SUM(COALESCE(oi.line_total_gross, oi.quantity * oi.unit_price_gross, 0)) FILTER (WHERE ${activeStatusPredicate('o.status')}),
               0
             )::numeric AS revenue
           FROM orders o
           JOIN order_items oi
           ...
           HAVING COALESCE(SUM(oi.quantity) FILTER (WHERE ${activeStatusPredicate('o.status')}), 0) > 0
