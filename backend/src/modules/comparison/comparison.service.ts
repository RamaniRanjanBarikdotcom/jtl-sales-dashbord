  ...
  async channels(scope: TenantScope, query: ComparisonQueryDto, maxLimit = 100) {
    ...
    const rows = await this.db.query(
      `WITH sales_products AS (
        ...
        SELECT COALESCE(cm.canonical_id, 'raw-' || md5(COALESCE(NULLIF(TRIM(o.channel), ''), 'Unknown'))) AS channel_id,
    ...
    const rows = await this.db.query(
      `WITH sales AS (
        ...
        WHERE oi.tenant_id = ANY($1::uuid[])
          AND o.order_date BETWEEN $2::date AND $3::date
          AND LOWER(COALESCE(o.status, '')) <> 'cancelled'
          AND COALESCE(cm.canonical_id, 'raw-' || md5(COALESCE(NULLIF(TRIM(o.channel), ''), 'Unknown'))) IN ($4, $5)
    ...
    const rows = await this.db.query(
      `WITH sales AS (
        SELECT oi.tenant_id, oi.product_id,
          ...
          COUNT(DISTINCT COALESCE(NULLIF(TRIM(o.channel), ''), 'Unknown'))::int AS channels,
          STRING_AGG(DISTINCT COALESCE(NULLIF(TRIM(o.channel), ''), 'Unknown'), ', ' ORDER BY COALESCE(NULLIF(TRIM(o.channel), ''), 'Unknown')) AS channel_names,
    ...
    const rows = await this.db.query(
      `WITH activity AS (
        SELECT o.tenant_id, o.customer_id,
          ...
          COUNT(DISTINCT COALESCE(NULLIF(TRIM(o.channel), ''), 'Unknown')) FILTER (WHERE o.order_date BETWEEN $6::date AND $7::date)::int AS channel_count,
