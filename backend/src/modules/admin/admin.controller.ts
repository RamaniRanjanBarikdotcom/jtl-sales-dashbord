...
export class AdminController {
  ...
  async getUsers(@Req() req: AuthenticatedRequest, @Query() query: PagedTenantScopeQueryDto) {
  ...
  async createUser(
    ...
    const scopedTenantId = await this.tenantContext.resolve(req, query.tenantId || bodyTenantId);
  ...
  async deleteUser(
    ...
    const scopedTenantId = await this.tenantContext.resolve(req, query.tenantId);
    return this.adminService.deleteUserPermanently(
      id,
      req.user.sub,
      req.user.role,
      scopedTenantId,
  ...
  deactivateTenant(@Param('id', new ParseUUIDPipe()) id: string, @Req() req: AuthenticatedRequest) {
    if (req.user.role !== 'super_admin') throw new ForbiddenException();
    return this.adminService.deactivateTenant(id, req.user.sub);
  }

  @Patch('tenants/:id/reactivate')
  @RequirePermissions(PERMISSIONS.ADMIN_MANAGE)
  reactivateTenant(@Param('id', new ParseUUIDPipe()) id: string, @Req() req: AuthenticatedRequest) {
    if (req.user.role !== 'super_admin') throw new ForbiddenException();
    return this.adminService.reactivateTenant(id, req.user.sub);
  }
  ...
  async rotateSyncKey(
    ...
    const targetTenantId = await this.tenantContext.resolve(
      req,
      req.user.role === 'super_admin' ? (query.tenantId || id) : undefined,
    );
    return this.adminService.rotateSyncKey(targetTenantId);
  }
