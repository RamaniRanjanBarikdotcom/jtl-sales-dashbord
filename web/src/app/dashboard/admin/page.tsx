...
function OneTimePasswordModal({
    ...
    return (
        <Modal title={title} onClose={onClose}>
            <p style={{ margin: "0 0 12px", fontSize: 12, color: DS.mid, lineHeight: 1.6 }}>
                Copy this temporary password now. It will not be shown again after you close this modal.
            </p>
            <div style={{
                background: "rgba(255,255,255,0.04)",
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
                <button onClick={copyPassword} style={BTN({ background: DS.sky, color: "#000" })}>
                    {copied ? "Copied" : "Copy password"}
                </button>
                <button onClick={onClose} style={BTN({ background: "rgba(255,255,255,0.06)", color: DS.mid })}>
                    Close
                </button>
            </div>
        </Modal>
...
function CreateUserModal({
    ...
    onClose: () => void;
    onPassword: (password: string, title: string) => void;
}) {
    ...
    const submit = async () => {
        ...
        try {
            ...
            if (result.temp_password) {
                onPassword(result.temp_password, `Temporary password — ${form.full_name}`);
            }
...
function ActionMenu({
    ...
    const handleResetPwd = async () => {
        setOpen(false);
        const result = await resetPwd.mutateAsync(user.id);
        if (result.temp_password) {
            onPassword(result.temp_password, `Temporary password — ${user.full_name}`);
        }
    };
    ...
    const handleResendInvite = async () => {
        ...
        if (result.temp_password) {
            onPassword(result.temp_password, `Invite password — ${user.full_name}`);
        }
    };
    ...
    const handleDeletePermanent = async () => {
        ...
        const firstConfirm = window.confirm(
            ...
            "This removes the user account, memberships, permissions, and active sessions from the database. This cannot be undone.",
        );
        if (!firstConfirm) return;
        const typed = window.prompt(`Type DELETE to permanently delete ${user.email}`);
        if (typed !== "DELETE") return;
        await deleteUser.mutateAsync({ id: user.id, tenantId: user.tenant_id });
    };
    ...
    const menuItems = [
        ...
        ...(canManagePermissions
            ...
            : []),
        { label: "✉ Resend invite",      action: handleResendInvite, color: DS.cyan },
        ...
        ...(user.id !== currentUserId && user.role !== "super_admin"
            ? [{ label: "🗑 Delete permanently", action: handleDeletePermanent, color: DS.rose }]
            : []),
    ];
            ...
            <>
                ...
                <div
                    ...
                    style={{
                        ...
                        padding: "5px", boxShadow: "0 12px 40px rgba(0,0,0,0.5)",
                    }}
                >
...
export default function AdminPage() {
    ...
    const [oneTimePassword, setOneTimePassword] = useState<{ password: string; title: string } | null>(null);
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                                    ...
                                    <tr key={u.id} style={{ borderBottom: `1px solid rgba(255,255,255,0.03)` }}>
                                        ...
                                        <td style={{ padding: "12px 10px", textAlign: "right" }}>
                                            <ActionMenu
                                                ...
                                                onPassword={(password, title) => setOneTimePassword({ password, title })}
            ...
            {/* Modals */}
            {showCreate && <CreateUserModal onClose={() => setShowCreate(false)} onPassword={(password, title) => setOneTimePassword({ password, title })} />}
