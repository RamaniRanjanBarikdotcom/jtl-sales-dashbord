...
function SecretValue({ label, value }: { label: string; value?: string | null }) {
    ...
    return (
        <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 10, color: DS.lo, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 5 }}>{label}</div>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <code style={{
                    flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis",
                    background: "rgba(255,255,255,0.05)", border: `1px solid ${DS.border}`,
                    borderRadius: 8, padding: "8px 10px", color: DS.hi, fontSize: 11,
                    fontFamily: DS.mono,
                }}>{value}</code>
                <button
                    onClick={() => navigator.clipboard?.writeText(value)}
                    style={BTN({ background: "rgba(56,189,248,0.12)", color: DS.sky })}
                >
                    Copy
                </button>
            </div>
...
function OnboardingSecretsModal({ result, onClose }: { result: CreateTenantResult | { sync_api_key?: string; sync_api_key_prefix?: string }; onClose: () => void }) {
    const tenant = "tenant" in result ? result.tenant : null;
    const firstAdmin = "firstAdmin" in result ? result.firstAdmin : null;
    return (
        <Modal title={tenant ? "Company Created" : "Sync Key Rotated"} onClose={onClose}>
            <div style={{
                ...
                Copy these values now. They are shown once and cannot be viewed again.
            </div>
            {tenant && (
                <div style={{ marginBottom: 12, fontSize: 12, color: DS.mid }}>
                    <strong style={{ color: DS.hi }}>{tenant.name}</strong>
            ...
            {result.sync_api_key_prefix && (
                <div style={{ fontSize: 10, color: DS.lo, marginTop: 4 }}>
                    Future screens show only prefix: <code style={{ color: DS.amber }}>{result.sync_api_key_prefix}…</code>
                </div>
            )}
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 18 }}>
                <button onClick={onClose} style={BTN({ background: DS.sky, color: "#001018" })}>I copied these</button>
            </div>
        </Modal>
    );
}
...
// ── create tenant modal ───────────────────────────────────────────────────────
function CreateTenantModal({ onClose, onCreated }: { onClose: () => void; onCreated: (result: CreateTenantResult) => void }) {
        ...
        <Modal title="Create New Tenant" onClose={onClose}>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                ...
                <button onClick={submit} disabled={createTenant.isPending}
                    style={BTN({ background: DS.orange, color: "#fff" })}>
...
function TenantMenu({ tenant }: { tenant: AdminTenant }) {
    ...
    const placeMenu = () => {
        ...
        const gap = 6;
        const viewportPad = 8;
        const topCandidate = (rect.bottom + gap + menuHeight <= window.innerHeight)
            ? rect.bottom + gap
            : Math.max(viewportPad, rect.top - menuHeight - gap);
        const top = Math.min(
            Math.max(viewportPad, topCandidate),
            Math.max(viewportPad, window.innerHeight - menuHeight - viewportPad),
        );
        const left = Math.min(
    ...
    const selectTenantAndGo = (path: string) => {
        const company = companies.find(c => c.tenantId === tenant.id) ?? {
            tenantId: tenant.id,
            membershipId: null,
            name: tenant.name,
            slug: tenant.slug,
        ...
        const nextCompanies = companies.some(c => c.tenantId === tenant.id)
            ? companies
            : [...companies, company];
        setCompanies(nextCompanies, company);
    ...
    const menuItems = [
        ...
        {
            label: "👥 Manage Users",
            action: () => selectTenantAndGo("/dashboard/admin"),
            color: DS.sky,
        },
        {
            label: "💊 View Sync Status",
            action: () => selectTenantAndGo("/dashboard/sync"),
            color: DS.violet,
        },
        { label: `🔑 Key Prefix: ${tenant.sync_key_prefix ?? "—"}`,  action: () => setOpen(false), color: DS.sky },
        {
            label: "↻ Rotate Sync Key",
            action: async () => {
                setOpen(false);
        ...
        tenant.is_active
            ...
            : { label: "✓ Reactivate",  action: () => { setOpen(false); reactivate.mutate(tenant.id); }, color: DS.emerald },
    ];
            ...
            <>
                ...
                <div
                    ...
                    style={{
                        ...
                        width: 220, maxWidth: "calc(100vw - 16px)", maxHeight: "calc(100vh - 16px)",
                        overflowY: "auto",
                        background: "#0d1e35", border: `1px solid ${DS.border}`, borderRadius: 9,
                        padding: 5, boxShadow: "0 12px 40px rgba(0,0,0,0.5)",
                    }}
                >
                    {menuItems.map((item, i) => (
                        <button key={i} onClick={item.action}
                            style={{ display: "block", width: "100%", textAlign: "left", background: "none", border: "none", cursor: "pointer", padding: "8px 11px", borderRadius: 6, fontSize: 12, color: item.color, fontFamily: "inherit", whiteSpace: "nowrap" }}>
                            {item.label}
                        </button>
                    ))}
                </div>
...
export default function SuperAdminPage() {
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {onboardingSecrets && (
                <OnboardingSecretsModal
                    result={onboardingSecrets}
                    onClose={() => setOnboardingSecrets(null)}
                />
            )}
        </div>
    );
