...
import {
  type OrdersTrendGranularity,
  type OrdersTrendPoint,
  useOrdersTrend,
} from "@/hooks/useOrdersTrend";
...
type TrendState = { granularity: OrdersTrendGranularity; range: TrendRange };
...
function normalizeRangeForGranularity(range: TrendRange, granularity: OrdersTrendGranularity): TrendRange {
...
function chooseGranularity(from: string, to: string): OrdersTrendGranularity {
...
export function OrdersTrendFullModal({ open, onClose }: Props) {
  const [mounted, setMounted] = useState(false);
  const [granularity, setGranularity] = useState<OrdersTrendGranularity>("year");
  ...
  const channelQ = useQuery({
    queryKey: ["overview", "orders-modal", "channels", scopedParams],
    enabled: open,
    queryFn: async () => {
  ...
  const paymentShipQ = useQuery({
    queryKey: ["overview", "orders-modal", "payment-shipping", scopedParams],
    enabled: open,
    queryFn: async () => {
  ...
  const countryQ = useQuery({
    queryKey: ["overview", "orders-modal", "countries", scopedParams],
    enabled: open,
    queryFn: async () => {
  ...
  const platformQ = useQuery({
    queryKey: ["overview", "orders-modal", "platforms", scopedParams],
    enabled: open,
    queryFn: async () => {
      const params = new URLSearchParams(scopedParams);
      params.set("locationDimension", "country");
  ...
  const onDrillDown = (point: OrdersTrendPoint) => {
    if (granularity === "year") {
  ...
  const onZoomChange = ({ start, end, points }: { start: number; end: number; points: OrdersTrendPoint[] }) => {
    if (debounceRef.current) clearTimeout(debounceRef.current);
  ...
  return createPortal(
    <>
      ...
      >
        ...
        >
          <div>
            <div style={{ fontSize: 16, color: DS.hi, fontWeight: 700 }}>Orders Trend Full View</div>
        ...
        <div style={{ padding: 16, overflow: "auto", display: "flex", flexDirection: "column", gap: 14 }}>
          {ordersTrendQ.isLoading ? (
            <div style={{ color: DS.lo, fontSize: 13 }}>Loading orders trend data...</div>
          ) : ordersTrendQ.isError || !data ? (
            <div style={{ color: DS.rose, fontSize: 13 }}>Failed to load orders trend data.</div>
          ) : (
            <>
              <OrdersSummaryCards summary={data.summary} />
              <div
                style={{
                  border: `1px solid ${DS.border}`,
                  borderRadius: 12,
              ...
              <div style={{ display: "grid", gap: 12, gridTemplateColumns: "minmax(360px, 1fr) minmax(520px, 1.4fr)" }}>
                <OrdersDetailsTable rows={data.points} granularity={granularity} />
