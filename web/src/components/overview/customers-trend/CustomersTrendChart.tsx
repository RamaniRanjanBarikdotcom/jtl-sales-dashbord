...
interface Props {
  points: CustomersTrendPoint[];
  onDrillDown: (point: CustomersTrendPoint) => void;
  onZoomChange: (payload: ZoomPayload) => void;
}

export function CustomersTrendChart({ points, onDrillDown, onZoomChange }: Props) {
  const option = {
    ...
    tooltip: {
      ...
      formatter: (params: Array<{ dataIndex: number }>) => {
        ...
        return [
          ...
          `Prior Year: ${row.priorCustomers.toLocaleString("en-US")}`,
          `YoY: ${yoy}`,
    ...
    legend: {
      data: ["Customers", "Prior Year Customers", "Revenue"],
    ...
    series: [
      {
        ...
        data: points.map((p) => p.customers),
      },
      {
        name: "Prior Year Customers",
        type: "line",
        smooth: true,
        symbolSize: 6,
        itemStyle: { color: DS.sky },
        lineStyle: { color: DS.sky, width: 2, type: "dashed" },
        data: points.map((p) => p.priorCustomers),
      },
      {
        name: "Revenue",
        type: "line",
        ...
        smooth: true,
        symbolSize: 5,
        itemStyle: { color: DS.emerald },
        lineStyle: { color: DS.emerald, width: 2 },
  ...
  return <ReactECharts option={option} onEvents={onEvents} notMerge lazyUpdate style={{ width: "100%", height: 460 }} />;
}
