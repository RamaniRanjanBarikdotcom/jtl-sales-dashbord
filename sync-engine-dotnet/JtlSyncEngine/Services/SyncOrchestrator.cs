...
#pragma warning restore CS0067
        ...
        // ─────────────────────────────────────────────────────────────────────
        public async Task SyncOrdersAsync(SyncModuleStatus status, CancellationToken ct = default, string? requestedSyncMode = null)
        {
        ...
        // ─────────────────────────────────────────────────────────────────────
        public async Task SyncProductsAsync(SyncModuleStatus status, CancellationToken ct = default, string? requestedSyncMode = null)
        {
        ...
        // ─────────────────────────────────────────────────────────────────────
        public async Task SyncCustomersAsync(SyncModuleStatus status, CancellationToken ct = default, string? requestedSyncMode = null)
        {
            ...
            string? requestedSyncMode = null)
        {
            ...
            {
                var fullSyncStart = new DateTime(2000, 1, 1, 0, 0, 0, DateTimeKind.Utc);
                var isRequestedFull = string.Equals(requestedSyncMode, "full", StringComparison.OrdinalIgnoreCase);
                var lastSyncTime = isRequestedFull && useSyncWindow
                    ? fullSyncStart
                    : _watermarks.GetLastSyncTime(module);
                ...
                var (resumeOffset, resumeWindowEnd) = isRequestedFull
                    ? (0, DateTime.MinValue)
                    : _watermarks.GetCheckpoint(module);
                var isResume = !isRequestedFull && resumeOffset > 0 && resumeWindowEnd > DateTime.MinValue;
                var syncEndTime = isResume ? resumeWindowEnd : DateTime.UtcNow;
                var syncMode = isRequestedFull || !useSyncWindow || lastSyncTime <= fullSyncStart.AddDays(1)
                    ? "full"
                    : "incremental";

                if (isResume)
                    _log.Info(module, $"Resuming sync from checkpoint: offset={resumeOffset}, window={lastSyncTime:yyyy-MM-ddTHH:mm:ssZ} → {syncEndTime:yyyy-MM-ddTHH:mm:ssZ}, mode={syncMode}, run={syncRunId}");
                else
                    _log.Info(module, $"Starting sync from {lastSyncTime:yyyy-MM-ddTHH:mm:ssZ}, mode={syncMode}, run={syncRunId}");
        ...
        private static string ComputeRowsChecksum(List<object> rows)
        {
